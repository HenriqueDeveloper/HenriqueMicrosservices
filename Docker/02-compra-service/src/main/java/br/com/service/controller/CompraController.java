...
import br.com.service.ConnectService.ConnectServiceCep;
...
public class CompraController {
	...
	@Autowired
	private ConnectServiceCep serviceCep;
	@Autowired
	...
	public Compra comprar(@PathVariable("nome") String nome, @PathVariable("cep") String cep) {
		...
		logradouro = serviceCep.BuscarCidade(cep);
		compra.setCep(cep);
		compra.setNomeProduto(prod.getNome());
		compra.setCidade(logradouro.getDescricao_cidade());
		compra.setPreco(prod.getPreco());
		
		if (logradouro.getUf().equals("PE")) {
